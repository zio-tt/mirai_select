...
const CharacterResponseDisplay = ({
  ...
  return (
    <div className="chat chat-start w-[90%] mb-3"
      ...
      <div className={`w-20 rounded-full chat-image avatar mr-2 p-1 ring ring-primary ring-offset-1 ${isResponse && isAnySelected ? ( isSelected  ? 'bg-white' : 'bg-gray-600' ) : '' }`}>
      ...
      <div className={`chat-header ${isResponse && isAnySelected ? ( isSelected  ? 'font-bold' : '' ) : '' }`}>
        {decisionCharacter.name}
      </div>
      <div className={chatStyle}>
        {characterResponse ? characterResponse.response : welcome}
      </div>
      {isResponse && isSelected && (
