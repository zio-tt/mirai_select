...
interface UserInterfaceProps {
  ...
  saveDecision: (e: React.MouseEvent<HTMLButtonElement>) => Promise<void>
}

const TagInputForm = ({ tags, setTags, saveDecision }: UserInterfaceProps) => {
  const { inputTags, setInputTags } = useHelper();
  const { displayTags, setDisplayTags } = useHelper();
  const { labelBgColor, setLabelBgColor } = useHelper();
  const { labelTextColor, setLabelTextColor } = useHelper();
  const { tagAlert, setTagAlert } = useHelper();

  const handleTagChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    // 入力値からスペースを含む全ての空白文字を取り除く
    const tagsWithoutSpaces = e.target.value.split(',')
