...
const OpeningAnimation = () => {
  ...
  const Content = () => (
    <FadeInOut onCompletion={() => setFadeInComplete(true)} animationType={animationType}>
      <div className='flex flex-col w-screen h-screen items-center justify-center relative overflow-none'>
        <h1 className='flex text-gray-300 text-3xl md:text-5xl lg:text-7xl'>
          ミライセレクト
        </h1>
        <h2 className='flex text-gray-500 text-lg md:text-2xl lg:text-4xl mt-3'>
          あなたの決断をサポートする
        </h2>
      </div>
    </FadeInOut>
  ...
  return <>{fadeInComplete ? <Content /> : <Content />}</>
}
